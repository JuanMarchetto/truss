...
fn test_schema_rule_valid_workflow_with_on() {
    ...
    for yaml in valid_workflows {
        ...
        let schema_errors: Vec<_> = result
            .diagnostics
            .iter()
...
fn test_schema_rule_non_github_actions_yaml() {
    ...
    let non_workflow_yamls = vec!["key: value", "database:\n  host: localhost\n  port: 5432"];
    ...
    for yaml in non_workflow_yamls {
        ...
        let schema_errors: Vec<_> = result
            ...
            .iter()
            .filter(|d| {
                d.message.contains("GitHub Actions")
...
fn test_schema_rule_deterministic() {
    ...
    let schema1: Vec<_> = result1
        .diagnostics
        .iter()
        .filter(|d| d.message.contains("on"))
        .collect();
    let schema2: Vec<_> = result2
        .diagnostics
        .iter()
        .filter(|d| d.message.contains("on"))
        .collect();
