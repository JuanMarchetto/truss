...
"#;
    ...
    let _concurrency_errors: Vec<_> = result.diagnostics
        .iter()
    ...
    assert!(
        analysis_succeeded,
        "Concurrency validation should process workflows without crashing"
    );
    
    // Future enhancement: When ConcurrencyRule is implemented, uncomment:
    // assert!(
    //     !_concurrency_errors.is_empty(),
...
"#;
    ...
    // Future enhancement: When ConcurrencyRule is implemented, uncomment:
    // assert!(
    //     !_concurrency_errors.is_empty() || result.diagnostics.iter().any(|d| d.message.contains("expression")),
