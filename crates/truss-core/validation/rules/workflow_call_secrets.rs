            ...
            if defined_secrets.is_empty() {
                ...
                diagnostics.push(Diagnostic {
                    message: format!(
                        ...
                        secret_name,
                        defined_secrets.iter().cloned().collect::<Vec<_>>().join(", ")
                    ),
                    severity: Severity::Error,
                    span,
                });
...
impl WorkflowCallSecretsRule {
        ...
        match node.kind() {
            "block_mapping_pair" | "flow_pair" => {
                if let Some(key_node) = node.child(0) {
                    ...
                    let secret_name = key_text.trim_matches(|c: char| c == '"' || c == '\'' || c.is_whitespace())
                        .trim_end_matches(':')
                        .to_string();
    ...
    fn validate_secret_properties(&self, node: Node, source: &str, diagnostics: &mut Vec<Diagnostic>) {
        match node.kind() {
            "block_mapping_pair" | "flow_pair" => {
                if let Some(key_node) = node.child(0) {
                    let key_text = utils::node_text(key_node, source);
                    ...
                    if let Some(secret_value_raw) = secret_value {
                        ...
                        let description_value = utils::find_value_for_key(secret_value, source, "description");
                        ...
                        while let Some(pos) = expr_lower[search_pos..].find("secrets.") {
                            ...
                            if !secret_name.is_empty() {
                                references.push((
                                    ...
                                    Span {
                                        start: i + 3 + actual_pos + 8,
                                        end: i + 3 + actual_pos + 8 + name_end,
                                    },
                                ));
                            }
