...
impl ValidationRule for WorkflowCallOutputsRule {
    ...
    fn validate(&self, tree: &Tree, source: &str) -> Vec<Diagnostic> {
        ...
        let (job_names, job_outputs) = if let Some(jobs_node) = jobs_value {
            (collect_job_names(jobs_node, source), collect_job_outputs(jobs_node, source))
        } else {
        ...
        if let Some(outputs_node) = outputs_value {
            ...
            for (expr_text, span) in all_expressions {
                ...
                let has_valid_pattern = expr_lower.contains("jobs.") && expr_lower.contains(".outputs.");
                            ...
                            if after_job.starts_with(".outputs") {
                                ...
                                if after_outputs_trimmed.starts_with(".") {
                                    ...
                                    if !output_name_cleaned.is_empty() {
                                        ...
                                        let job_name_start = expr_offset_in_source + jobs_pos_in_expr + 5; // +5 for "jobs."
